...
- **文本引导机制**：利用CLIP模型的文本-图像对齐能力，通过文本描述增强多模态特征学习
...
- 🔄 **多模态融合**：同时处理高光谱图像（HSI）和LiDAR数据
- 📝 **文本引导**：利用CLIP模型实现文本-图像对齐，增强特征表示
- 🔗 **跨模态交互**：通过CMIM_MBAM模块实现HSI和LiDAR之间的深度交互
- 🎯 **多数据集支持**：支持houston2013、muufl、trento、augsburg、berlin等多个数据集
- 🚀 **端到端训练**：支持完整的训练和测试流程
- 📊 **详细评估**：提供OA、Kappa、AA、CA等多种评估指标
